...
#         # Add 1 to avoid issues with zero or negative values
#         transformed_col, _ = boxcox(X[:, col] + 1)
#         transformed_columns.append(transformed_col)
    
#     return np.array(transformed_columns).T

# # ColumnTransformer to apply transformations to specific features
# preprocessor = ColumnTransformer(
#     transformers=[
#         ('log_transform', FunctionTransformer(log_transform), ['HbA1c_level', 'blood_glucose_level']),
#         ('boxcox_transform', FunctionTransformer(boxcox_transform, validate=False), ['bmi']),
#         ('pass_through', 'passthrough', ['age', 'gender', 'smoking_history']),  # Pass other features as is
#     ]
# )
# silhoutte_score=[]
# for i in range(2,11):
...
__all__ = ['X_train', 'y_train', 'X_test', 'y_test', 'preprocessor']
